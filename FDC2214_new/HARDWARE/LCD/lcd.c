...
u16 LCD_ReadPoint(u16 x, u16 y)
{
    ...
    if (lcddev.id == 0X9325 || lcddev.id == 0X4535 || lcddev.id == 0X4531 || lcddev.id == 0X8989 ||
        lcddev.id == 0XB505) { return r; } //这几种IC直接返回颜色值
...
void LCD_Set_Window(u16 sx, u16 sy, u16 width, u16 height)
{
    ...
    twidth = sx + width - 1;
    theight = sy + height - 1;
    if (lcddev.id == 0X9341 || lcddev.id == 0X5310 || lcddev.id == 0X6804 || (lcddev.dir == 1 &&
                                                                              lcddev.id == 0X1963))
    {
        LCD_WR_REG(lcddev.setxcmd);
        LCD_WR_DATA(sx >> 8);
...
void LCD_Init(void)
{
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC | RCC_APB2Periph_GPIOB | RCC_APB2Periph_AFIO,
                           ENABLE); //使能PORTB,C时钟和AFIO时钟
    ...
    else if (lcddev.id == 0x6804) //6804初始化
    {
        ...
        LCD_WR_DATAX(0X09);
        LCD_WR_REG(
            0XD2);//Operational Amplifier Circuit Constant Current Adjust , charge pump frequency setting
        LCD_WR_DATAX(0X01);
        LCD_WR_DATAX(0X22);
    ...
    else if (lcddev.id == 0x9320) //测试OK.
    {
        ...
        delay_ms(50);
        LCD_WriteReg(0x10, (1 << 12) | (0 << 8) | (1 << 7) | (1 << 6) | (0 <<
                                                                         4)); //Power Control 1.(0x16b0)
