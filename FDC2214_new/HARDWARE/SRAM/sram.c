...
void FSMC_SRAM_Init(void)
{
    ...
    GPIO_InitStructure.GPIO_Pin = 0xDF33;               //PORTD复用推挽输出
    ...
    GPIO_InitStructure.GPIO_Pin = 0x023F;               //PORTD复用推挽输出
    ...
    readWriteTiming.FSMC_DataSetupTime =
        0x03;       //数据保持时间（DATAST）为3个HCLK 4/72M=55ns(对EM的SRAM芯片)
    ...
    FSMC_NORSRAMInitStructure.FSMC_Bank =
        FSMC_Bank1_NORSRAM2;//  这里我们使用NE2 ，也就对应BTCR[4],[5]。
...
void FSMC_SRAM_ReadBuffer(u8 *pBuffer, u32 ReadAddr, u32 n)
{
    for (; n != 0; n--)
    {
        *pBuffer++ = *(vu8 *)(Bank1_SRAM2_ADDR + ReadAddr);
